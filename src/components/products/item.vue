...
<template>
  <dl class="s-item">
    ...
    <dd>
      <h3>
        <router-link :to="{name:'producrDetail'}">{{meta.title}}</router-link>
      </h3>
      <p>
      <el-rate
        v-model="meta.score"
      ...
      <span v-if="meta.score > 4">较好</span>
      <span v-if="meta.score > 3.5">一般</span>
      <span v-if="meta.score > 3">不错</span>
      <span v-else>差</span>
      <span class>{{meta.score}}分</span>
      <span class="s-item-comment-total">{{meta.num}}人评论</span>
      </p>
      <p>
